...
const form = (state, dispatch) => {
  ...
  return html`
    <form method="POST" onsubmit=${handleForm(dispatch, { type: 'measure:editFormSaved', oldShortId: measure.short_id })} onkeyup=${handleForm(dispatch, { type: 'measure:editFormChanged' })} onchange=${handleForm(dispatch, { type: 'measure:editFormChanged' })}>
      ...
      <div class="field">
        <label for="fulltext" class="label has-text-grey">Measure Text</label>
        <div class="control">
          <textarea name="fulltext" autocomplete="off" class="textarea" rows="4" placeholder="The full text of the measure." required value="${fulltext || ''}"></textarea>
        </div>
      </div>
