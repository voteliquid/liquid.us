...
module.exports = (state, dispatch) => {
  ...
  return html`
    <form method="POST" onsubmit=${handleForm(dispatch, { type: 'measure:editFormSaved', oldShortId: measure.short_id })} onkeyup=${handleForm(dispatch, { type: 'measure:editFormChanged' })} onchange=${handleForm(dispatch, { type: 'measure:editFormChanged' })}>
      ...
      <div class="field">
        <label for="summary" class="label has-text-grey">Introduction</label>
        <div class="control">
          <textarea name="comment" autocomplete="off" class="textarea" rows="10" placeholder="Briefly explain your policy proposal and why you support it." required value="${measure.comment || ''}"></textarea>
        </div>
      </div>
      <div class="field">
