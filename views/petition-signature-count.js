...
module.exports = ({ vote_position, vote_counts = [] }) => {
  const count = vote_counts[0] && vote_counts[0].yeas

  return html`
    <div>
      <p>
        <span class="has-text-weight-bold">${count} signature${count > 1 ? 's' : ''}</span>${vote_position ? `, including yours.` : '.'}
