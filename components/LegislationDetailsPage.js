...
module.exports = class LegislationDetailsPage extends Component {
  oninit() {
    ...
    return this.api(url).then((bills) => {
      ...
      const title = selected_bill.introduced_at ? `${selected_bill.type} ${selected_bill.number} – ${selected_bill.title}` : selected_bill.title
  ...
  fetchComments(selected_bill) {
    return this.api(`/public_votes?measure_id=eq.${selected_bill.id}&order=proxy_vote_count.desc.nullslast,created_at.desc`)
    .then(comments => ({
